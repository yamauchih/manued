...
%% Copyright (C) 1998-2002 Hitoshi Yamauchi 山内 斉
%%
...
\begin{abstract}
 ...
 manued method and \texttt{manued.el} which is an emacs lisp
 implementation for supporting \texttt{manued} method. I wish this
 method and \texttt{manued.el} will be a help for the people who feels
...
manuscript via Email.  I present \texttt{manued.el} as one of the
implementations to assist the method method.

\subsection{Installation}

\texttt{manued.el} depends on the Emacs version 19 or later. please
use \texttt{manued.el} on the Emacs version 19.34 or later, or mule
based on them. Author uses it on the GNU Emacs 19.34.1, Mule Version
2.3 (SUETSUMUHANA). Installation of \texttt{manued.el} is as follows.
...
To try to \texttt{manued.el}, you can read the tutorial
file(\texttt{manued.tut}) or directly enter to the
\texttt{manued-minor-mode}.

\begin{itemize}
\item Read the tutorial \\ %
  You can find the \texttt{manued-e.tut} file in the archive of the
...
\item Directly enter to the \texttt{manued-minor-mode} \\ %
  ...
  \item To insert manued swap command, put a mark and type M-x
    manued-insert-swap-command (or type C-c C-m C-s).
  \item To extract revised document in another buffer, Type
    manued-show-newer-in-manued-buffer.
  \item Others: please select a menu item. Type M-n or M-p, this is
...
\htmladdnormallink{TAKEUCHI, Ikuo}{http://ne.cs.uec.ac.jp/tak.html} in
No.39 Programming symposium in Japan~\cite{bib:Takeuchi1998}. Manued
...
via Email.(\htmladdnormallink{Here is original text in
  Japanese on the Web.}{./manuscripteditting.txt.gz})
...
\item We usually can not use colors.
\item Some mail reader does not keep two dimensional information of
  ...
  occur.
\item We do not want to depend on line feed position. To send exact
...
\cite{bib:Takeuchi1998} says that proofreading for an electric text
...
useful. Therefore, I implemented \texttt{manued.el}, this program
supports manued method. \texttt{manued.el} has next functions :
...
\item defescape\\ %
  ...
  \verb|~|[foo\verb|~|]/These are \verb|~|[bars\verb|~|]] will be
...
\end{verbatim}
  ...
  $a_n/b_n$ and ] is corresponding to manued command symbol. But these
...
\item defversion\\ %
  ...
  \texttt{manued.el}.  This is useful information for the
  \texttt{manued.el} program.  (default value `(manued-version-num)')
\end{itemize}
...
used. \texttt{Manued.el} checks consistency of def-commands and warns
...
\item Supplement \\ %
  ...
  symbol. For example, \verb|~[| will be \verb|[|.
\end{itemize}
...
\verb|manued-pretty-print-null-comment-out-p|.
Here, we describe only about delete command, but also the way to control
swap command with pretty print mode. See \ref{sec:PrettyPrintCustomize}
in details.


\subsection{Example of manued documents}
...
\begin{itemize}
\item \htmladdnormallink{http://www.mpi-sb.mpg.de/\~{}hitoshi/otherprojects/manued/index.shtml}{http://www.mpi-sb.mpg.de/~hitoshi/otherprojects/manued/index.shtml}
\item \htmladdnormallink{http://www.archi.is.tohoku.ac.jp/\~{}yamauchi/otherprojects/manued/index.shtml}{http://www.archi.is.tohoku.ac.jp/~yamauchi/otherprojects/manued/index.shtml}
\item \htmladdnormallink{http://ne.cs.uec.ac.jp/}{http://ne.cs.uec.ac.jp/} (Mirror site)
\end{itemize}
...
colors), extracting revised text and original text. \texttt{manued.el}
...
I (the author of \texttt{manued.el}) have been interested in Lisp
...
not written any Lisp programs before writing \texttt{manued.el}. I
...
offer \texttt{manued.el} here. His name is MAEDA Atusi. I thank him.


\subsection{Installation}

\texttt{manued.el} is based on Emacs Version 19.34 or later. Please
...
\textit{Manued.el} provides two main customizing methods. One of them
...
When you set next variables in \texttt{.emacs} file or set by any
...
\subsubsection{Range specification of manued document}

%\textbf{This section will be obsolete in the future version.}
...
\subsubsection{Colors of Hi-Light}
% <label id="HilightColor">

There are three choices for using color. Set following variable to
\texttt{manued-use-color-hilit} according to your wish.

\begin{itemize}
\item Use color as possible as it can.(\texttt{t})
\item Never use color.(\texttt{nil})
\item According to font-lock-mode status.(\texttt{'follow-font-lock-mode})
\end{itemize}
...
\begin{itemize}
\item The quality of this document is very poor. Suggestions and
  Comments are welcome.
\item Current version (0.9.5) does not use \texttt{font-lock-mode},
...
\texttt{manued.el} (C) Copyright 1998-2012 Yamauchi, Hitoshi
...
\subsection{Manued Mailing List}
...
\end{quotation}
\latexhtml{}{(Please delete last .nicht.spam part and substitute `` at
...
How to compile TeX file which extracted by show-newer-command
immediately.
...
instead of `\verb|[|' and `\verb|]|'. Next setting is one example.
...
However this seems not so good solution. If you find some good strings
for manued method, please inform me.
...
this. \texttt{manued.el} may not be written without his assistance.
...
\begin{itemize}
 \item KAWAMURA, Takao (bug fix etc. for version 0.9.3)
 \item NAKAJIMA, Mikio (bug fix etc. for version 0.9.3)
 \item Stefan Monnier  (comments for version 0.9.1)
 \item Suzuki, Ken-ichi (manuediff)
%% 回答待ち
%%\item SUZUKI, Shingo (Maintainer of manued mirror pages)
%%\item TANEMURA Shin'ichi  (Maintainer of manued mirror pages)
\end{itemize}
...
\begin{rawhtml}
<address>
Copyright (C) 1998-2002 yamauchi, Hitoshi<br>
